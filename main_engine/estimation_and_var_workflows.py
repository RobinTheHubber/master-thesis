...
from datasets.estimated_parameters import dic_estimated_parameters_marginal_skewed_t_constant_mean, dic_estimated_parameters_realEGARCH
...
from main_engine.get_data import load_return_models_and_data, load_realized_cov, load_return_data
...
def estimation_and_var_workflow():
    ...
    marginal_models_estimated = {
     ...
     #    skewed_t: dic_estimated_parameters_marginal_skewed_t_constant_mean
        'realEGARCH': dic_estimated_parameters_realEGARCH}
    ...
    for distribution_marginal, dictionary_estimated_parameters_marginal in marginal_models_estimated.items():
        # marginal_models_list, T, daily_returns = load_return_models_and_data(distribution_marginal, train_idx, mean_equation, n)
        marginal_models_list = None
        daily_returns = load_return_data(n)
        daily_realized_cov, rv = load_realized_cov(n)
        T = daily_returns.shape[0]
        test_set_size = T - train_idx
        beta, realized_estimates = HAR_estimation_workflow(0, train_idx)

        dictionary_parameter_estimates, dictionary_filtered_rho0 = retrieve_model_estimation_output(copula_type, cpar_equation, marginal_models_list,
                                         train_idx, dictionary_estimated_parameters_marginal, realized_estimates, n,
                                         daily_returns, rv, run_model)

        ## set value-at-risk parameters
        portfolio_returns = (daily_returns.values @ weights).reshape((-1,))
        realized_measure = get_realized_measure(n, daily_realized_cov, rv, (train_idx, T))
        _, h_ij_hat_test = HAR_estimation_workflow(from_=train_idx, to_=T+1)

        retrieve_VaR_estimation_output(portfolio_returns, rv, copula_type, cpar_equation, marginal_models_list,
                                       train_idx, h_ij_hat_test, weights,
                                       N,
...
def retrieve_VaR_estimation_output(portfolio_returns, rv, copula_type, cpar_equation, marginal_models_list,
                                   train_idx, realized_measure, weights, N,
    ...
    if run_model:
        ...
        value_at_risk_output = value_at_risk_workflow(marginal_models_list, test_set_size,
                                                                                  daily_returns,
                                                                                  weights, N, portfolio_returns, rv,
...
def retrieve_model_estimation_output(copula_type, cpar_equation, marginal_models_list,
                                     train_idx, dictionary_estimated_parameters_marginal, daily_realized_cov, n, data, list_sigma2, run_model=False):
