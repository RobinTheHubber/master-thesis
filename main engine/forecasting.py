# todo forecasting distribution of returns via MC simulation (goal: simulate the set of returns for 1 period in the future many times)
#  -> filtering parameters for the copula's one time step ahead
#  -> input filtered parameters for simulating from the vine copula to get the PITs
#  -> input PITs and filtered marginal volatility and mean to get simulated marginals
#  -> repeat above steps many times and obtain Value-At-Risk figures from quantiles of the simulated marginals
...
def compute_value_at_risk(data, cpar_equation, copula_type, marginal_models_list, dicParameters_estimated, test_set_size, N=1000):
    T = data.shape[0]
    ...
    for j in range(n):
        PITs_j = get_PITs_with_estimated_parameters(data.iloc[:, j], dicParameters_estimated[j + 1], marginal_models_list[j])
    ...
    dictionary_filtered_rho_test_set = get_filtered_rho_after_estimation(PITs, h_function, dicParameters_estimated, n, cpar_equation, rho0)
    ...
    for i in range(int(N/N_)):
        PITs_test_set[i*N_:(i+1)*N_, :, :] = sample_from_vine3D(dictionary_h, dictionary_h_inv, dictionary_filtered_rho_test_set, n, test_set_size, N_)
...
def compute_value_at_risk_test_output(data, value_at_risk):
    ...
    for q in Q:
        for j in range(n):
            dictionary_VaR_test_results[(q, j)] = compute_test_value_at_risk(data.values[:, j], q, value_at_risk[q][:, j])
...
dic_estimated_parameters_all_gaussian = {1  :  {'dist': None, 'mean': np.array([0.08986738]), 'vol': [8.875398354470813e-05, 0.016544337693967393, 0.9834384776660482]},
...
def test_get_v():
    v = get_filtered_rho_after_estimation(np.random.random(size=2000).reshape(400, 5), h_function_gaussian, dic_estimated_parameters_all_gaussian, 5, 'product')
    print(v)