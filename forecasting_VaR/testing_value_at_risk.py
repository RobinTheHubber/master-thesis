...
from forecasting_VaR.forecasting import get_value_at_risk_output_single_garch, get_value_at_risk_output_HS, \
    get_value_at_risk_output_MV
...
from marginal_engine.distributions import gaussian, student_t
...
def analyze_value_at_risk_output():
    portfolio_returns = get_portfolio_returns()
    T = len(portfolio_returns)
    test_set_size = T - RunParameters.estimation_window
    ...
    loop_over_run_parameters(store_var_output_in_dictionary, dictionary_all_var_output)
    ...
    for q in Q:
        plt.figure(100*q)
        plt.plot(portfolio_returns[-test_set_size:])
        ...
        for j, item in enumerate(dictionary_all_var_output.items()):
            ...
            var_test_output = compute_value_at_risk_test_output(portfolio_returns[-test_set_size:], var_outpout)
